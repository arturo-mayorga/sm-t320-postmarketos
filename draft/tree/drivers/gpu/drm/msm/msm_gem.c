...
void msm_gem_purge(struct drm_gem_object *obj)
{
	...
	/*
	 * Objects in the VRAM carveout (no IOMMU) are private GEM objects
	 * with no shmem file behind them: obj->filp is NULL and put_pages()
	 * above has already handed the carveout range back. Touching the
	 * shmfs here oopses inside the shrinker, and the dying task takes the
	 * object's resv lock with it -- the compositor's next madvise then
	 * sleeps in D state forever.
	 */
	if (!obj->filp)
		return;
