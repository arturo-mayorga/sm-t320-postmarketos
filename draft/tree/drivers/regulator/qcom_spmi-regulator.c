...
spmi_regulator_find_range(struct spmi_regulator *vreg)
{
	...
	/*
	 * mondrianwifi: PM8841 S5 (the Krait APC rail) reports a range the
	 * table does not know. Dump what the hardware holds so the decode can
	 * be fixed, rather than failing registration silently.
	 */
	{
		u8 hdr[8] = { 0 }, vol[8] = { 0 };

		spmi_vreg_read(vreg, 0x00, hdr, 8);
		spmi_vreg_read(vreg, 0x40, vol, 8);
		dev_warn(vreg->dev,
			 "%s: unknown range_sel 0x%02x (logical type %d); regs 00..07: %02x %02x %02x %02x %02x %02x %02x %02x, 40..47: %02x %02x %02x %02x %02x %02x %02x %02x\n",
			 vreg->desc.name, range_sel, vreg->logical_type,
