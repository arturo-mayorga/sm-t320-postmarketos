...
/*
 * Some WCNSS firmware (Samsung's 2017 build for the wcn3680 in the Galaxy Tab
 * Pro 8.4: "WCN v2.0 RadioPhy vIris_TSMC_3.0", API 1.5.1.2) advertises
 * SCAN_OFFLOAD but never answers UPDATE_CHANNEL_LIST or START_SCAN_OFFLOAD,
 * so every scan times out. scan_offload=0 takes the legacy mac80211 software
 * scan path that the vendor prima driver used on that firmware.
 */
static bool scan_offload = true;
module_param(scan_offload, bool, 0644);
MODULE_PARM_DESC(scan_offload, "Use firmware scan offload when advertised (default: Y)");
